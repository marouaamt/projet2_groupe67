...
def click2():
    ...
    def openFile():
        filePath = filedialog.askopenfilename(filetypes= "*.csv"), ("*.xlsx")
    ...
    def S1(d, a, d1): return 1 / (1 + np.exp(-a * (d - d1)))
    def S2(d, a, d2): return 1 / (1 + np.exp(-a * (d - d2)))
    ...
    def analyze_and_plot(f, h_tx, h_rx, slope_threshold, d_user, loss_user, d_min, d_max):
        ...
        for idx, (start, end) in enumerate(segments):
            ...
            plt.plot(d_seg, lp_seg, color=colors[idx % len(colors)], linewidth=2, label=f"{environnements[idx]} ({start}-{end})")
